    ...
    <div className="relative w-8 h-12 lg:w-12 lg:h-18">
      ...
      {drawnCard && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="absolute -top-6 lg:-top-8 left-1/2 -translate-x-1/2 text-white text-[10px] lg:text-xs font-bold whitespace-nowrap"
        >
        ...
        <AnimatePresence>
          {drawnCard ? (
            <AnimatedCard
              ...
              className="w-8 h-12 lg:w-12 lg:h-18 absolute inset-0"
              onClick={onCardClick}
              isSelectable={isSelectable}
            />
          ) : (
            // Empty slot placeholder
            <div className="w-8 h-12 lg:w-12 lg:h-18 border-2 border-dashed border-white/20 rounded-lg flex items-center justify-center text-center">
              <span className="text-white/30 text-[10px] lg:text-xs">Draw Slot</span>
            </div>
          )}
        </AnimatePresence>
      </div>
      ...
      {isSelectable && (
        <motion.p
          ...
          animate={{ opacity: 1 }}
          transition={{ delay: 1 }}
          className="absolute -bottom-6 lg:-bottom-8 left-1/2 -translate-x-1/2 text-green-400 text-[10px] lg:text-xs whitespace-nowrap"
        >
          Click to play
        </motion.p>
      )}
