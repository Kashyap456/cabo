...
class RedisManager:
    ...
    async def pop_message_non_blocking(self, room_id: str) -> Optional[Dict[str, Any]]:
        """Pop message from game queue (non-blocking)"""
        await self.ensure_connected()
        key = f"queue:game:{room_id}:messages"
    ...
    async def publish_event(self, room_id: str, event: Dict[str, Any]) -> str:
        ...
        event_id = await self._redis.xadd(
            stream_key,
            {"event": json.dumps(event), "timestamp": str(
                event.get('timestamp', ''))}
        )
    ...
    async def cleanup_game(self, room_id: str) -> None:
        ...
        logger.info(
            f"Cleaned up {len(keys_to_delete)} keys for room {room_id}")
